...
function mkExecMock({ psHasDaemon = true, pluginsHasA2a = true, pluginsLoaded = true, httpRoutes = 1 } = {}) {
  return async function execImpl(cmd, args) {
    ...
    if (cmd === 'bash' && String(args[0]) === '-lc' && String(args[1]).includes('ps aux | grep run_agent_loop')) {
      return { stdout: psHasDaemon ? 'yes\n' : 'no\n', stderr: '' };
    }
...
cases.push(
  await runCase('daemon_missing', {
    execImpl: mkExecMock({ psHasDaemon: false, pluginsHasA2a: true, pluginsLoaded: true, httpRoutes: 1 }),
    fetchImpl: mkFetchMock({ status: 200 })
  })
);

// 2) simulate plugin missing/not loaded
cases.push(
  await runCase('plugin_missing', {
    execImpl: mkExecMock({ psHasDaemon: true, pluginsHasA2a: false, pluginsLoaded: false, httpRoutes: 0 }),
    fetchImpl: mkFetchMock({ status: 200 })
  })
);
...
cases.push(
  await runCase('gateway_route_down', {
    execImpl: mkExecMock({ psHasDaemon: true, pluginsHasA2a: true, pluginsLoaded: true, httpRoutes: 1 }),
